...
Самый короткий способ — фасад $modx->mxl (с версии 1.2.0): доступен сразу из любого сниппета/плагина/чанка, getService вызывать не нужно. Тот же $modx->mxl работает и в версии под MODX 3 — API вызовов одинаков в обеих версиях. Если фасад недоступен (версия ниже 1.2.0), сервис также доступен как $modx->mxlogger (extension_packages). Для краткости в примерах ниже используем $mxl:
<code>$mxl = $modx->mxl;
